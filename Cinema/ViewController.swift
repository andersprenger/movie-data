...
class ViewController: UITableViewController {
    ...
    func allMovies() -> [Movie]? {
        do {
            return try persistentContainer.viewContext.fetch(Movie.fetchRequest())
        } catch {
    ...
    private func populate() {
        ...
        let actor4 = Actor(context: persistentContainer.viewContext)
        actor4.name = "Robert Downey Jr."
        actor4.gender = "Male"
        actor4.birthDate = dateHelper(day: 12, month: 12, year: 1970)
        
        let actor5 = Actor(context: persistentContainer.viewContext)
        actor5.name = "Tom Holland"
        actor5.gender = "Male"
        actor5.birthDate = dateHelper(day: 01, month: 03, year: 2001)
        
        let actor6 = Actor(context: persistentContainer.viewContext)
        actor6.name = "Zendaya"
        ...
        let actor7 = Actor(context: persistentContainer.viewContext)
        actor7.name = "Robert Pattinson"
        actor7.gender = "Male"
        actor7.birthDate = dateHelper(day: 4, month: 9, year: 1980)
        
        let actor8 = Actor(context: persistentContainer.viewContext)
        actor8.name = "Zoë Kravitz"
        actor8.gender = "Male"
        actor8.birthDate = dateHelper(day: 01, month: 01, year: 1940)
        
        let actor9 = Actor(context: persistentContainer.viewContext)
        ...
        // *******************************************************
        let pc1 = ProductionCompany(context: persistentContainer.viewContext)
        pc1.name = "Disney"
        pc1.country = "US"
        pc1.website = "disney.com"
        pc1.movies = NSSet(array: [movie2, movie3, movie5])

        let pc2 = ProductionCompany(context: persistentContainer.viewContext)
        pc2.name = "Warner"
        pc2.country = "US"
        pc2.website = "warner.com"
        pc2.movies = NSSet(array: [movie1])
        
        let pc3 = ProductionCompany(context: persistentContainer.viewContext)
        pc3.name = "Sony"
        pc3.country = "JP"
